...
- 무음 모드(ADR 0012): 소리를 낼 수 없는 자리를 위해 마이크를 켜지 않고 핵심 키워드를 적는
  ...
  낮게 나오기 때문이다. 같은 이유로 무음 시도는 최고·최근 유사도를 갱신하지 않고
  `missedKeyPoints`만 갱신해 약점 덱을 살린다.

- 답변 방식 전환을 학습 화면 안에 뒀다. 소리를 낼 수 있는 자리인지는 하루에도 여러 번 바뀐다.
  마이크 권한 거부·미지원 안내에도 무음으로 바꾸는 길을 함께 뒀다. 지금까지는 막다른 길이었다.
  기본값은 프로필 화면에서 정하며 기본은 그대로 소리 내어 말하기다.

- 오늘의 10초 구술 카드가 오늘 끝냈는지를 표시한다. 하루 한 문항에 끝맺음이 생긴다.
  날짜별 집계를 새로 저장하지 않고 그 문항의 마지막 확인 시각에서 센다.
