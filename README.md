...
현재 버전은 `1.0.0` MVP다. 서버, 로그인, 실시간 AI 호출, 광고 SDK 없이 198개의 versioned 질문과 기기 내 진도만 사용한다. Sentry는 DSN을 설정한 출시 후보에서 JavaScript 오류만 받는 opt-in adapter다.
...
- 지하철이나 사무실에서는 무음 모드로 바꿔 마이크 없이 핵심 키워드만 적는다. 학습 화면에서
  바로 바꿀 수 있고, 이때는 유사도 대신 핵심 포인트 적중만 센다.
- 마스코트 `텐이`, 연속 학습, 5개 성장 레벨과 0–100 설명력 점수를 제공한다.
- 용어집 검색, 카테고리 필터, 약한 질문 다시 보기를 제공한다.
- 자기평가와 복습 횟수로 다시 볼 시점을 정해, 잊을 때쯤 그 문항이 세션 앞에 다시 나온다.
- 학습·용어집·기록·프로필의 네 화면을 floating glass navigation으로 이동한다.
- 모든 화면에서 라이트·다크·기기 설정을 바로 바꾼다. 신규 사용자는 라이트가 기본이고 이후 선택은 기기에 유지한다.

설명력 점수는 현재 198개 질문 전체를 분모로 `알았다=1`, `애매했다=0.5`, `몰랐다=0`을 합산해 반올림한다. 시험·지능·절대 실력 점수가 아니라 이 기기에 기록된 학습 진도다.
