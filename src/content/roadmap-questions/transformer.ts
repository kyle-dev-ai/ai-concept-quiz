...
export const transformerRoadmapQuestions = [
  ...
  {
    id: 'transformer-attention-intuition',
    category: 'transformer',
    difficulty: 'foundation',
    term: 'Attention · Weighting',
    prompt: '문장에서 "그것"이 무엇을 가리키는지 모델은 어떻게 알아내나요?',
    shortAnswer:
      '앞에 나온 단어들을 다시 봅니다. 전부 똑같이 보는 게 아니라 관련 있는 단어에 더 큰 비중을 두는데, 이렇게 어디를 얼마나 볼지 비중으로 정하는 장치가 attention입니다.',
    deepAnswer:
      '비중을 매기는 기준은 사람이 정해주지 않고 학습으로 얻어집니다. 문장마다 봐야 할 곳이 다르므로 비중 자체는 매번 새로 계산되고, 그래서 같은 단어라도 문장이 다르면 다르게 해석됩니다. 멀리 떨어진 단어도 한 번에 볼 수 있다는 점이 앞에서부터 차례로 읽는 방식과 가장 크게 다릅니다.',
    keyPoints: [
      '어디를 얼마나 볼지 비중으로 정한다',
      '비중 매기는 기준은 학습으로, 비중은 문장마다 새로 계산',
    ],
    followUp: '문장이 길어질수록 볼 곳도 많아지는데 계산량은 어떻게 될까요?',
    prerequisites: [],
  },
  {
    id: 'transformer-contextual-meaning',
    category: 'transformer',
    difficulty: 'foundation',
    term: 'Contextual Embedding',
    prompt: '"배가 아프다"와 "배를 탔다"의 "배"를 모델은 어떻게 구분하나요?',
    shortAnswer:
      '단어마다 고정된 뜻을 하나씩 들고 있는 게 아닙니다. 같은 문장 안의 다른 단어들을 보고 그 자리의 표현을 새로 만들기 때문에, 같은 글자라도 문장이 다르면 다른 값이 됩니다.',
    deepAnswer:
      '이렇게 문맥에 따라 달라지는 표현을 contextual embedding이라고 하며, 단어마다 값이 하나로 고정된 예전 방식과 구분됩니다. 층을 지날수록 주변 정보가 더 섞여 들어가므로 위층으로 갈수록 문맥이 더 반영됩니다. 다만 사람이 해석할 수 있는 뜻이 숫자 하나하나에 대응하는 것은 아닙니다.',
    keyPoints: [
      '표현이 단어가 아니라 그 자리에서 만들어진다',
      '같은 글자라도 문장이 다르면 다른 값이 된다',
    ],
    followUp: '사전처럼 단어마다 값이 하나로 고정돼 있으면 어떤 문장에서 문제가 생길까요?',
    prerequisites: ['transformer-attention-intuition'],
  },
  {
