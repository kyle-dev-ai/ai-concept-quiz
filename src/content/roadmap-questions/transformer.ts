...
export const transformerRoadmapQuestions = [
  ...
  },
  {
    ...
    shortAnswer:
      '어휘 크기가 크면 두 행렬이 전체 파라미터의 상당 부분을 차지하는데, 하나로 묶으면 그만큼을 줄일 수 있습니다. 또 어떤 token을 입력으로 이해하는 공간과 출력으로 예측하는 공간을 같은 공간으로 묶어 규제 효과도 얻습니다.',
    deepAnswer:
  ...
  },
  {
    ...
    deepAnswer:
      '한 후보에 확률을 전부 몰면 logit 격차를 무한정 키우는 방향으로 학습이 흘러 과확신과 과적합이 생기는데, 목표 분포를 살짝 눕히면 이 압력이 줄어듭니다. 확률값 자체를 의사결정에 쓰는 서비스에서는 확신도가 눌리는 부작용이 있으므로 calibration을 따로 확인해야 하고, 정도가 지나치면 정답과 오답의 구분이 흐려집니다.',
    keyPoints: [
  ...
  },
  {
    ...
    shortAnswer:
      'FFN을 여러 전문가로 복제해두고 router가 token마다 그중 소수만 골라 통과시키기 때문입니다. 전체 파라미터는 전문가 수만큼 늘지만 한 token이 실제로 지나는 경로는 몇 개뿐이라 연산량은 크게 늘지 않습니다.',
    deepAnswer:
