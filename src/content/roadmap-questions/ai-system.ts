...
export const aiSystemRoadmapQuestions = [
  ...
  },
  {
    ...
    deepAnswer:
      '채점 기준을 문장으로만 주면 일관성이 낮으므로, 항목별 채점 기준과 예시를 제시하고 점수를 매기기 전에 근거를 먼저 쓰게 하는 편이 안정적입니다. 무엇보다 사람 라벨 일부와 대조해 채점자 자체의 정확도를 먼저 검증해야 하며, 이 검증 없이 나온 점수는 근거가 약합니다. 두 답 중 하나를 고르는 비교 방식이 절대 점수보다 안정적인 경우가 많지만 비교 횟수가 후보 수의 제곱으로 늘고 순서 편향은 오히려 더 크므로, 순서를 바꿔 두 번 묻고 평균 내는 처리가 사실상 필수입니다.',
    keyPoints: [
