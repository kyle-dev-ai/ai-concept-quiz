...
export const aiSystemRoadmapQuestions = [
  {
    ...
    shortAnswer:
      'Exact cache는 같은 정규화 입력과 설정일 때만 재사용하고, semantic cache는 embedding이 비슷한 질문에도 과거 응답을 재사용합니다.',
    deepAnswer:
      'Semantic 방식은 hit를 늘리지만 의도가 다른 질문에 엉뚱한 답을 돌려줄 위험이 큽니다. Model·prompt·retrieval index·권한·locale version을 cache key에 반영하고 TTL과 invalidation을 설계합니다. 개인정보나 사용자별 context가 섞인 응답은 공유 cache에서 분리하며 hit rate뿐 아니라 stale·wrong-hit 품질을 측정합니다.',
    keyPoints: [
  ...
  },
  {
    ...
    shortAnswer:
      '무제한 queue는 대기시간과 memory를 계속 늘려 결국 전체 장애를 만듭니다. 사용자별 rate limit, bounded queue, concurrency limit으로 부하를 앞단에 알려야 합니다.',
    deepAnswer:
      '저우선 요청을 빠르게 거절해야 핵심 요청을 보호할 수 있습니다. 429와 Retry-After 같은 명확한 contract를 제공하고 client retry에는 jitter를 넣어 재시도 폭주를 막습니다. Token 수와 tool 비용처럼 요청별 무게가 다르므로 단순 request count 외에 동시 실행과 비용 quota도 고려합니다.',
  ...
  },
  {
    ...
    shortAnswer:
      'Model, prompt, parameter, tool schema, retrieval index와 dataset version, 입력 식별자, 실행 단계와 결과를 서로 연결해 기록해야 합니다.',
    deepAnswer:
      '비결정적 sampling이면 seed와 반복 trial도 함께 남깁니다. 원문 개인정보를 무조건 저장하지 않고 hash, redaction, 접근 통제와 보존 기간을 적용합니다. Trace replay로 변경 전후를 같은 입력에 실행하고 fingerprint로 실험 구성을 고정해야 원인과 회귀를 설명할 수 있습니다.',
    keyPoints: [
  ...
  },
  {
    ...
    shortAnswer:
      '사용자 task의 성공 기준과 baseline을 먼저 정의하고, offline golden set과 end-to-end 성공률, latency·cost·안전 지표를 함께 말합니다.',
    deepAnswer:
      'component별 metric, 실패 유형, 개선 전후 수치까지 있으면 방어가 훨씬 단단해집니다. Dataset 출처와 split, 대표성, 반복 trial, version을 설명하고 online feedback이 실제 목표와 어떻게 연결되는지 말해야 합니다. 좋은 결과만 제시하지 않고 한계와 다음 falsifiable experiment를 제시하면 연구·운영 사고를 보여줄 수 있습니다.',
