...
export const ragRoadmapQuestions = [
  ...
  },
  {
    ...
    deepAnswer:
      '이 숫자 배열을 벡터라고 부르고, 그래서 이런 검색을 벡터 검색이라고 합니다. 표기가 달라도 뜻이 비슷하면 찾아낸다는 점이 키워드 검색과 가장 다릅니다. 대신 제품 코드나 사람 이름처럼 글자 그대로 맞아야 하는 검색에는 오히려 약해서, 실무에서는 키워드 검색과 함께 쓰는 경우가 많습니다.',
  ...
  },
  {
    ...
    shortAnswer:
      '앞쪽 차원만 잘라 써도 되도록 학습된 Matryoshka 계열이어야 성능이 유지되고, 자른 벡터는 길이가 1이 아니게 되므로 다시 정규화해야 합니다.',
    deepAnswer:
      '코사인은 크기에 영향받지 않지만 내적이나 유클리드 거리는 벡터마다 길이가 달라지면 코사인과 순위가 어긋납니다. 차원을 줄이면 저장 용량과 검색 속도는 좋아지지만 미세한 의미 구분이 흐려져 상위 순위가 뒤바뀔 수 있으므로, 줄인 차원에서 검색 품질 지표를 다시 측정해야 합니다. 절단을 전제로 학습되지 않은 모델이라면 그냥 자르지 말고 PCA 같은 차원 축소를 쓰는 편이 안전합니다. 정규화 여부와 거리 함수는 짝이 맞아야 하고, 색인은 정규화하고 질의는 안 하는 불일치가 흔한 실수이며, 색인된 벡터의 규칙을 바꾸면 전체 재색인이 필요합니다.',
    keyPoints: [
      '차원 절단을 전제로 학습된 모델인지 먼저 확인',
      '모든 벡터가 단위 길이일 때만 내적·유클리드 순위가 코사인과 일치',
    ],
